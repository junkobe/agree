package com.agree.base.redis;

import com.agree.base.ConfigHelper;
...
public class JedisWritePoolUtil {
    ...
    private static JedisWritePoolUtil instance = null;
    private static ReentrantLock lock = new ReentrantLock();

    private JedisWritePoolUtil(){

    }

    public static JedisWritePoolUtil getInstance(){
        if(instance==null){
            lock.lock();
            if(instance==null){
//                System.out.println(lock.toString());
                instance = new JedisWritePoolUtil();
            }
            lock.unlock();
        }
        return instance;
    ...
    private void initJedisPool() {
        properties = ConfigHelper.loadProperties("writeConfig.properties");
        ...
        String password = StringUtil.nullToString(properties.getProperty("redis.pool.password"));
        int timeout = Integer.parseInt(properties.getProperty("redis.pool.timeout"));
        String masterName = StringUtil.nullToString(properties.getProperty("redis.pool.mastername"));
//        jedisPool = new JedisSentinelPool(masterName,getIPs(),config,password);//(masterName, getIPs(), config);
        jedisPool = new JedisSentinelPool(masterName,getIPs(),config,timeout,password);
    }

    private Set<String> getIPs() {
        Set<String> sentinels = new HashSet<String>();
        String ips = properties.getProperty("sentinels.ips");
        String ports = properties.getProperty("sentinels.ports");
        List<String> ipList = StringUtil.stringSpiltToList(ips, CONFIGS.spileValArg);
        List<String> portList = StringUtil.stringSpiltToList(ports, CONFIGS.spileValArg);
        for(int i=0;i<ipList.size();i++){
            sentinels.add(ipList.get(i)+":"+portList.get(i));
        }
...
//            logger.error("请指定jedis pool的配置文件路径！");
//            return null;
//        }else{
//            return getJedis(propertyPath);
//        }
//    }
    ...
    private Jedis getJedis() {
        if (jedisPool == null) {
            ...
            if(jedisPool == null){
                initJedisPool();
                logger.info("JeidsPool init success!");
            }
            lock.unlock();
        }
...
//    public Transaction getTranc(){
//        return getJedis().multi();
//    }
//
//    public List<Object> execTranc(Transaction tx){
//        List<Object> results = tx.exec();
//        closeJedis(getJedis());
//        return results;
//    }
//
//    public String discardTranc(Transaction tx){
//        String results = tx.discard();
//        closeJedis(getJedis());
//        return results;
//    }
//
//    public String watchTranc(Transaction tx,String ...keys){
//        tx.watch(keys);
//        closeJedis(getJedis());
//        return "";
//    }
    ...
    private final Luas luas = new Luas();
    ...
    public class Pipe{
        ...
        public void hmset(String key,Map<String,String> value){
            ...
            Response<String> response = pl.hmset(key,value);
            pl.sync();
            closeJedis(jedis);
//            return response;
        }

        public void hmset(Map<String,Map<String,String>> data){
            Jedis jedis = getJedis();
//            jedis.flushDB();
            Pipeline pl = jedis.pipelined();
...
//        }
    ...
    public class Scan{
        ...
        public ScanResult<String> scan(String cursor, String match){
            Jedis jedis = getJedis();
    ...
    public class Luas{
         ...
         * @return
         */
        public Object luaeval(String script,List<String> keys,List<String> args){
            Jedis jedis = getJedis();
//            try {
//                List<String> rs = new ArrayList<String>();

            Object results = jedis.eval(script, keys, args);

//                if( results == null)
//                    return null;
//                else if (String.class.isInstance(results))
//                    rs.add((String) results);
//                else if (List.class.isInstance(results)) {
//                    return (List<String>) results;
//                } else {
//                    rs.add(results.toString());
//                }
            closeJedis(jedis);
            return results;
//            }finally {
    ...
    public class Hash {
        ...
        public long hset(String key,String[] fields,String[] values){
            ...
            for(int i = 0;i<fields.length;i++){
                s = jedis.hset(key,fields[i],values[i]);
                if(s == 0){
                    break;
                }
            }
